...
2. Set your Path of Exile filter folder in `.env` (optional on Windows).

```env
FILTER_PATH="C:\Users\user\Documents\My Games\Path of Exile"
```

On Windows the default `<Documents>/My Games/Path of Exile` is used automatically when `FILTER_PATH` is not set. On other platforms you must set it explicitly.
