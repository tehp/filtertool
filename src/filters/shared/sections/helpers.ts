...
const SLOT_NAME_MAP: Record<SocketableItemClass, string> = {
...
function linkTtsFile(linkCount: 3 | 4, itemClass: SocketableItemClass): string {
  const prefix = linkCount === 3 ? "Three" : "Four"
  const slot = SLOT_NAME_MAP[itemClass]
  return soundFileTTS(`${prefix} Link ${slot}`)
}
...
export const buildItemClassSocketRules = ({
  ...
  itemClasses.map((itemClass) => {
    ...
    if (linkedSockets === 3 || linkedSockets === 4) {
      return builtRule.tts(linkTtsFile(linkedSockets, itemClass))
    }
...
export const buildGoodFourLinkRules = ({ defenceType, maxAreaLevel }: GoodFourLinkConfig) =>
  ARMOUR_CLASSES.flatMap((itemClass) => {
    ...
    const ruleWithSound = buildBaseRule().tts(linkTtsFile(4, itemClass))
...
export const LEVELING_AMULETS = {
  Amber: { baseType: "Amber Amulet", text: "Amber Amulet" },
  Jade: { baseType: "Jade Amulet", text: "Jade Amulet" },
  Lapis: { baseType: "Lapis Amulet", text: "Lapis Amulet" },
} as const satisfies Record<string, { baseType: Autogenerated.Amulets; text: string }>
...
export const buildTierCurrency = (
  ...
  compileRules(
    rule(
      ...entries.map(({ baseTypes, iconColor, iconShape, soundId, tts }) => {
        const builtRule = rule()
          .baseType(...baseTypes)
...
export const buildFlaskRule = ({
  ...
  return builtRule.tts(soundFileTTS(text))
}
...
export const buildFlaskSeries = ({
  ...
  entries: Array<{ baseTypes: string[]; maxAreaLevel?: number; text: string }>
}) =>
...
export const buildUtilityFlaskRules = (entries: Array<{ baseType: string; text: string }>) =>
