...
} as const satisfies Record<string, { baseType: Autogenerated.Amulets; entry: SoundManifestEntry }>
...
const buildHighlightedRule = ({
  ...
  if (tts) {
    builtRule.tts(typeof tts === "string" ? soundFileTTS(tts) : manifestSoundFile(tts))
  ...
  compileRules(
    rule(
      ...entries.map(({ baseTypes, iconColor, iconShape, soundId, tts }) => {
        ...
        if (tts) {
          builtRule.tts(manifestSoundFile(tts))
        } else if (soundId !== undefined) {
          builtRule.sound(soundId)
        }
